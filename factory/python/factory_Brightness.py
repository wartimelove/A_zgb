...
_UNTESTED = '等待中...'
_INQUIRE = '测试是否通过？'
brightness = '/sys/class/backlight/i915_backlight/brightness'
...
def test_brightness_fun(widget, event):
    os.system('echo 0 >' + brightness)
    time.sleep(1)    
    os.system('echo 100 >' + brightness)
    time.sleep(1)    
    os.system('echo 256 >' + brightness)
    time.sleep(1)
    os.system('echo 200 >' + brightness)
    time.sleep(1)

    return False

_LED_TEST_LIST = [
    ('brightness','明亮度', lambda *x: test_brightness_fun(*x))]

class factory_Brightness():
    version = 1
    ...
    def __init__(self):
        ...
        prompt_label = gtk.Label('明亮度测试,按空格键开始测试\n'
                                 '失败按Tab键，通过按回车键\n')
